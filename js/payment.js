  ...
  Swal.fire({
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        insertToDB(id_product, user, namaProduct, quantityPerPerson, totalPrice);
      }
    })
    .catch((error) => {
      // Handle fetch error
...
function insertToDB(id_product, user, namaProduct, quantityPerPerson, totalPrice) {
  return fetch('../../config/divingPayment.php', {
    ...
    .then((dataRes) => {
      ...
      fetchDivingPayment(name, totalPrice, orderId);
    })
    .catch((error) => {
...
function fetchDivingPayment(name, total, transaksiId) {
  Swal.fire({
